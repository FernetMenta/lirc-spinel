...
****************************************************************************
*
* Lirc driver for Spinel ir receiver
*
...
static int spinel_init(void);
static int spinel_deinit(void);
static char* spinel_rec(struct ir_remote* remotes);
static int spinel_decode(struct ir_remote* remote, struct decode_ctx_t* ctx);
static void* spinel_repeat(void* arg);
static int spinel_getcode(void);
...
/* spinel USB iR Receiver */
const struct driver hw_spinel = {
	.name		= "spinel",
	...
	.code_length	= 32,
	.init_func	= spinel_init,
	.deinit_func	= spinel_deinit,
	.open_func	= default_open,
	.close_func	= default_close,
	.send_func	= NULL,
	.rec_func	= spinel_rec,
	.decode_func	= spinel_decode,
...
const struct driver* hardwares[] = { &hw_spinel, (const struct driver*)NULL };
...
int spinel_init(void)
{
	log_info("spinel initializing '%s'", drv.device);
	...
	if (pthread_create(&repeat_thread, NULL, spinel_repeat, NULL)) {
 ...
 *  specific code.
 */
char* spinel_rec(struct ir_remote* remotes)
{
	unsigned ev;
	int rd;
